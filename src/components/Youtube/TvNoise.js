...
const TvNoise = props => {
  ...
  useEffect(() => {
    ...
    window.onresize = function() {
      ...
      samples = [];
      for (var i = 0; i < SAMPLE_COUNT; i++) samples.push(generateRandomSample(context, canvas.width, canvas.height));
    };
    ...
    function generateRandomSample(context, w, h) {
      ...
      var intensityCurve = [];
      for (let i = 0; i < Math.floor(h / factor) + factor; i++) intensityCurve.push(Math.floor(Math.random() * 15));
  ...
  return (
    <div className={`noise-container ${(videoId || currentVideo) && status !== STATUS_BUFFERING && 'fade-out'}`}>
